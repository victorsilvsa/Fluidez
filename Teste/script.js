    ...
    ${entries.length === 0 ? '<div class="card" style="text-align:center;color:#94a3b8">Nenhuma matéria-prima. Toque + para criar.</div>' :
      ...
      <div style="display:flex;justify-content:space-between;align-items:center">
        <div>
          ...
          <div style="font-size:12px;color:#94a3b8;margin-top:2px">Índice de Fluidez: ${m.ifMin} — ${m.ifMax} </div>
        </div>
  ...
  `;
    ...
    function renderLoadDetail() {
      ...
      <div style="font-size:13px;font-weight:600;margin-bottom:8px;color:#94a3b8">Gráfico Índice de Fluidez por Palete</div>
    ...
    <div class="card">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:13px">
        ...
        <div><span style="color:#94a3b8">Faixa Índice de Fluidez:</span> ${mat ? mat.ifMin + ' — ' + mat.ifMax : 'N/A'}</div>
        <div><span style="color:#94a3b8">Média Índice de Fluidez:</span> ${s.avg ? s.avg.toFixed(2) : '—'}</div>
      </div>
    </div>
    ${chartHTML}
    ...
    ${paletes.length === 0 ? '<div class="card" style="text-align:center;color:#94a3b8">Nenhum palete. Toque + para adicionar.</div>' :
        ...
        <div>
          ...
          <div style="font-size:12px;color:#94a3b8">${p.date} · Índice de Fluidez: ${p.ifValue}  </div>
        </div>
  ...
  `;
    ...
    <form id="matForm" style="display:flex;flex-direction:column;gap:12px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div><label style="font-size:12px;color:#94a3b8;display:block;margin-bottom:4px">Índice de Fluidez Mínimo</label><input class="input-field" id="matMin" type="number" step="0.01" required placeholder="0.00"></div>
        <div><label style="font-size:12px;color:#94a3b8;display:block;margin-bottom:4px">Índice de Fluidez Máximo</label><input class="input-field" id="matMax" type="number" step="0.01" required placeholder="0.00"></div>
      </div>
      ...
      document.getElementById('matForm').onsubmit = e => {
        ...
        if (ifMin >= ifMax) { toast('Índice de Fluidez mínimo deve ser menor que máximo', true); return; }
    ...
    <form id="paleteForm" style="display:flex;flex-direction:column;gap:12px">
      ...
      <div><label style="font-size:12px;color:#94a3b8;display:block;margin-bottom:4px">Valor Índice de Fluidez ( )</label><input class="input-field" id="palÍndice de Fluidez" type="number" step="0.01" required placeholder="0.00"></div>
      ...
      document.getElementById('paleteForm').onsubmit = e => {
        ...
        const ifValue = parseFloat(document.getElementById('palÍndice de Fluidez').value);
    ...
    function generatePDF(id) {
      ...
      const info = [
        ...
        ['Faixa Índice de Fluidez', mat ? mat.ifMin + ' — ' + mat.ifMax + '  ' : 'N/A']
      ];
      info.forEach(([k, v]) => {
      ...
      if (paletes.length > 0) {
        doc.autoTable({
          startY: y,
          head: [['#', 'Data Análise', 'Índice de Fluidez ( )', 'Status']],
      ...
      doc.setFont(undefined, 'normal');
      doc.setFontSize(10);
      doc.text(`Média Índice de Fluidez: ${s.avg.toFixed(2)}   | ${s.approved}/${s.total} aprovados (${s.pct}%)`, 20, y + 18);
